...
def compute_profile_raysampling(img, x0, y0, scale=False, **kwargs):
    ...
    scale_factor = rays_mu.max() + 1e-5 if scale else 1
    rays_mu = rays_mu / scale_factor
    rays_se = np.nanstd(np.abs(rays), 0) / np.sqrt(scale_factor)
    return rays_mu, rays_se


def extract_rays(img, x0, y0, step=10, n_samples=200):
    n = int(n_samples / 4)  # Ensure n is an integer
    H, W = img.shape
    H, W = H / 2, W / 2
    x = np.random.randint(0, W - 1, n).tolist() + np.random.randint(0, W - 1, n).tolist() + np.zeros(n, dtype=int).tolist() + np.repeat(W - 1, n).tolist()
    y = np.zeros(n, dtype=int).tolist() + np.repeat(H - 1, n).tolist() + \
